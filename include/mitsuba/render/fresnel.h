...
NAMESPACE_BEGIN(mitsuba)
    ...
    template<typename Float>
    Float fresnel_ext(Float cosThetaI_, Float& cosThetaT_, Float eta) {
        if (drjit::any(!eta == 1)) {
        ...
        if (drjit::any(cosThetaTSqr <= 0.0f)) {
            cosThetaT_ = 0.0f;
            return 1.0f;
        }

        /* Find the absolute cosines of the incident/transmitted rays */
        Float cosThetaI = dr::abs(cosThetaI_);
        Float cosThetaT = dr::sqrt(cosThetaTSqr);
