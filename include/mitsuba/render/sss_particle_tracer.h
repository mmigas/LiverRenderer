...
template <typename Float, typename Spectrum> struct ScatterSamplingRecord { // Record used at runtime by samplers abstracting SS scattering
    Point<Float, 3> p;
    Vector<Float, 3> n;
    ...
    DRJIT_STRUCT(ScatterSamplingRecord, p, n, outDir, isValid, throughput, sampledColorChannel)
};

template <typename Float, typename Spectrum>
std::tuple<Ray<Point<Float, 3>, Spectrum>, Vector<Float, 3>> sampleShape(const Shape<Float, Spectrum> *shape, const MediumParameters<Spectrum> &medium,
...
template <typename Float, typename Spectrum>
std::tuple<Ray<Point<Float, 3>, Spectrum>, Vector<Float, 3>> sampleShapeFixedInDir(const Shape<Float, Spectrum> *shape, Sampler<Float, Spectrum> *sampler,
                                                                                   const Vector<Float, 3> inDirLocal) {
    Float time                           = 0.0f;
    PositionSample<Float, Spectrum> pRec = shape->sample_Position(time, sampler->next2D());
...
template <typename Float> inline Vector<Float, 3> sampleHG(const Vector<Float, 3> &d, const Point<Float, 2> &sample, float g) {
    Float cosTheta;
...
template <typename Float, typename Spectrum> class Volpath3D : public Object {
public:
    ...
    static drjit::Matrix<Float, 3> azimuthSpaceTransform(const Vector3f &refDir, const Vector3f &normal) {
        ...
        drjit::Matrix<Float, 3> rotMat({ std::cos(phi), std::sin(phi), 0.f }, { -std::sin(phi), std::cos(phi), 0.f }, { 0.f, 0.f, 1.f });
    ...
    static drjit::Matrix<Float, 3> azimuthSpaceTransformNew(const Vector3f &light_dir, const Vector3f &normal) {
        ...
        if (drjit::any_or<true>(drjit::abs(drjit::dot(normal, light_dir)) > 0.99999f)) {
            ...
            drjit::Matrix<Float, 3> lsTransform(s.x(), s.y(), s.z(), t.x(), t.y(), t.z(), light_dir.x(), light_dir.y(), light_dir.z());
            return lsTransform;
        }
        drjit::Matrix<Float, 3> lsTransform(t1.x(), t1.y(), t1.z(), t2.x(), t2.y(), t2.z(), light_dir.x(), light_dir.y(), light_dir.z());
        return lsTransform;
    }
    ...
    static Float effectiveAlbedo(const Float &albedo) { return -drjit::log(1.0f - albedo * (1.0f - drjit::exp(-8.0f))) / 8.0f; }
    ...
    static Spectrum effectiveAlbedo(const Spectrum &albedo) {
        float r      = dr::slice(effectiveAlbedo(albedo[0]), 0);
        float g      = dr::slice(effectiveAlbedo(albedo[1]), 0);
        float b      = dr::slice(effectiveAlbedo(albedo[2]), 0);
        Spectrum ret = Spectrum(r, g, b);
        ret          = xyz_to_srgb(ret); // TODO: Check if this is correct xyz_to_srgb in the original is fromLinearRGB
        return ret;
    }

    static Interaction<Float, Spectrum> intersectPolynomial(const Ray<Point3f, Spectrum> &ray, const Polynomial<Float> &polynomial, Float stepSize,
        ...
        for (int i = 0; i < 50 / stepSize; ++i) {
            ...
            float polyValue = evalPoly(polynomial.refPos, ray.o + ray.d * t, polynomial.order, polynomial.scaleFactor, polynomial.useLocalDir,
            ...
            if (i == 0) {
                ...
                if (polyValue * initVal <= 0) {
                    ...
                    // Assuming we are in a regime where the derivative makes sense, perform newton-bisection iterations to find the actual intersection
                    // location
                    ...
                    for (int j = 0; j < 5; ++j) {
                        ...
                        std::tie(polyValue, deriv) = evalPolyGrad(polynomial.refPos, ray.o + ray.d * t, polynomial.order, pX, pY, pZ, polynomial.scaleFactor,
                                                                  polynomial.useLocalDir, polynomial.refDir, polynomial.coeffs);
