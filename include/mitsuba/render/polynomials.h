...
template <typename _PointType, typename _DataRecord> struct SimpleKDNode {
    ...
    inline SimpleKDNode(const DataRecord &data) : position((Scalar) 0), right(0), data(data), flags(0) {}
...
template <typename Float, typename Spectrum> class MI_EXPORT_LIB ConstraintKdTree {
...
public:
    ...
    void build(const std::vector<Point3f> &sampledP, const std::vector<Vector3f> &sampledN) {
    ...
    std::tuple<std::vector<Point3f>, std::vector<Vector3f>, std::vector<Float>> getConstraints(const Point3f &p, Float kernelEps, const std::function<Float(Float, Float)> &kernel) const {
...
private:
    std::vector<Point3f> m_globalPoints;
    std::vector<Vector3f> m_globalNormals;
    // CTree m_tree;
    // std::tuple<Point3f, Vector3f, size_t> avgValues(TreeNode &node, TreeNode::IndexType index);
    /*std::pair<Float, Float> getMinMaxDistanceSquared(const Point3f &p, const AABB &bounds) const;
    void getConstraints(const Point3f &p, TreeNode node, TreeNode::IndexType index, const AABB &aabb, std::vector<Point> &points, std::vector<Vector> &normals,
                        std::vector<Float> &sampleWeights, Float kernelEps, const std::function<Float(Float, Float)> &kernel) const;*/
};
...
inline size_t numPolynomialCoefficients(size_t deg) { return (deg + 1) * (deg + 2) * (deg + 3) / 6; }

static constexpr int nChooseK(int n, int k) { return (k == 0 || n == k) ? 1 : nChooseK(n - 1, k - 1) + nChooseK(n - 1, k); }

static constexpr int nPolyCoeffs(int polyOrder) { return nChooseK(3 + polyOrder, polyOrder); }

static constexpr int nPolyCoeffs(int polyOrder, bool hardSurfaceConstraint) { return hardSurfaceConstraint ? nChooseK(3 + polyOrder, polyOrder) - 1 : nChooseK(3 + polyOrder, polyOrder); }
template <typename Float, typename Spectrum> class PolyUtils {
protected:
    ~PolyUtils() = default;
...
public:
    ...
    static std::tuple<std::vector<std::vector<Point3f>>, std::vector<std::vector<Vector3f>>, std::vector<std::vector<float>>> getLocalPoints(const std::vector<Point<Float, 3>> &queryLocations, Float kernelEps, const std::string &kernel,
                                                                                                                                             const ConstraintKdTree<Float, Spectrum> *kdtree) {
        ...
        for (size_t k = 0; k < queryLocations.size(); ++k) {
            ...
            std::vector<Float> sampleWeights;
            std::tie(positionConstraints, normalConstraints, sampleWeights) = kdtree->getConstraints(queryLocations[k], kernelEps, kernelFun);
    ...
    static void basisFunFitBuildA(const Point3f &pos, const Vector3f &inDir, const std::vector<Point3f> &evalP, const Eigen::VectorXi &permX, const Eigen::VectorXi &permY, const Eigen::VectorXi &permZ, const Eigen::VectorXf *weights,
                                  Eigen::Matrix<float, Eigen::Dynamic, nPolyCoeffs(polyOrder, hardSurfaceConstraint)> &A, const Eigen::VectorXf &weightedB, Float scaleFactor) {
        size_t n = evalP.size();
        ...
        Eigen::Matrix<float, Eigen::Dynamic, nCoeffs> fullA = Eigen::Matrix<float, Eigen::Dynamic, nCoeffs>::Zero(n * 4, nCoeffs);
        size_t termIdx                                      = 0;
        for (size_t d = 0; d <= polyOrder; ++d) {
            for (size_t i = 0; i <= d; ++i) {
                size_t dx = d - i;
                for (size_t j = 0; j <= i; ++j) {
                    size_t dy           = d - dx - j;
                    size_t dz           = d - dx - dy;
                    Eigen::VectorXf col = relPosPow.col(0 + 3 * dx).array() * relPosPow.col(1 + 3 * dy).array() * relPosPow.col(2 + 3 * dz).array();
                    if (weights) {
    ...
    template <size_t polyOrder, bool hardSurfaceConstraint = true> std::tuple<Polynomial<Float>, std::vector<Point3f>, std::vector<Vector3f>> fitPolynomialsImpl(const PolyFitRecord<Float, Spectrum> &pfRec, const ConstraintKdTree<Float, Spectrum> *kdtree) {
        ...
        if (useLightSpace) {
            basisFunFitBuildA<polyOrder, hardSurfaceConstraint>(Point3f(0.0f), usedRefDir, positionConstraints, pX, pY, pZ, &weights, A, weightedB, fitScaleFactor);
        } else {
    ...
    static std::tuple<Polynomial<Float>, std::vector<Point3f>, std::vector<Vector3f>> fitPolynomial(const PolyFitRecord<Float, Spectrum> &pfRec, const ConstraintKdTree<Float, Spectrum> *kdtree) {
    ...
    static Vector3f evalGradient(const Point3f &pos, const dr::Array<Float, 20> &coeffs, const ScatterSamplingRecord<Float, Spectrum> &sRec, size_t degree, Float scaleFactor, bool useLocalDir, const Vector3f &refDir) {
    ...
    static void projectPointsToSurface(const Scene *scene, const Point3f &refPoint, const Vector3f &refDir, ScatterSamplingRecord<Float, Spectrum> &sRec, const drjit::Array<Float, 20> &polyCoefficients, size_t polyOrder, bool useLocalDir, Float scaleFactor,
                                       Float kernelEps) {
        if (!sRec.isValid)
            return;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            Wavelength wl;
            Ray3f ray1 = Ray3f(sRec.p, dir, maxProjDist, 0, wl);
            ...
            //if (dr::any_or<true>(its.is_valid())) {
                ...
                //Log(Info, "Found intersection");
            //}
            Float maxT                = itsFoundCurrent ? its.t : maxProjDist;
            Ray3f ray2                = Ray3f(sRec.p, -dir, -maxT, 0, wl);
    ...
    static Float getKernelEps(const MediumParameters<Spectrum> &medium, int channel = 0, Float kernel_multiplier = 1.0f) {
    ...
    template <size_t degree> static float evalPolyImpl(const Point3f &pos, const Point3f &evalP, Float scaleFactor, bool useLocalDir, const Vector3f &refDir, const std::vector<float> &coeffs) {
        assert(degree <= 3);
    ...
    static float evalPoly(const Point3f &pos, const Point3f &evalP, size_t degree, Float scaleFactor, bool useLocalDir, const Vector3f &refDir, const std::vector<float> &coeffs) {
    ...
    template <typename T>
    static std::pair<Float, Vector3f> evalPolyGrad(const Point3f &pos, const Point3f &evalP, size_t degree, const int *permX, const int *permY, const int *permZ, Float scaleFactor, bool useLocalDir, const Vector3f &refDir, const T &coeffs) {
        Vector3f relPos;
        if (useLocalDir) {
            Vector3f s, t;
            Volpath3D<Float, Spectrum>::onbDuff(refDir, s, t);
    ...
    static Float getFitScaleFactor(const MediumParameters<Spectrum> &medium, int channel = 0, Float kernel_multiplier = 1.0f) { return getFitScaleFactor(getKernelEps(medium, channel, kernel_multiplier)); }

    static Float getFitScaleFactor(Float kernelEps) { return 1.0f / drjit::sqrt(kernelEps); }
    ...
    template <size_t PolyOrder = 3> static void legendreTransform(Eigen::Matrix<Float, nPolyCoeffs(PolyOrder), 1> &coeffs) {
        if (PolyOrder == 2) {
            float t[4]     = { 2.8284271247461903, 0.9428090415820634, 0.9428090415820634, 0.9428090415820634 };
            int indices[4] = { 0, 4, 7, 9 };

            float t2[9] = { 1.632993161855452, 1.632993161855452, 1.632993161855452, 0.8432740427115678, 0.9428090415820634, 0.9428090415820634, 0.8432740427115678, 0.9428090415820634, 0.8432740427115678 };
            Float val   = 0.0f;
            for (int i = 0; i < 4; ++i) {
            ...
            float t2[16]      = { 0.8432740427115678,  0.9428090415820634,  0.9428090415820634, 0.8432740427115678,  0.9428090415820634, 0.8432740427115678,  0.427617987059879,   0.48686449556014766,
                                  0.48686449556014766, 0.48686449556014766, 0.5443310539518174, 0.48686449556014766, 0.427617987059879,  0.48686449556014766, 0.48686449556014766, 0.427617987059879 };
    ...
    template <int order> static void rotatePolynomial(std::vector<float> &c, std::vector<float> &c2, const Vector<Float, 3> &s, const Vector<Float, 3> &t, const Vector<Float, 3> &n) {
        c2[0] = c[0];
        c2[1] = c[1] * s[0] + c[2] * s[1] + c[3] * s[2];
        c2[2] = c[1] * t[0] + c[2] * t[1] + c[3] * t[2];
        c2[3] = c[1] * n[0] + c[2] * n[1] + c[3] * n[2];
        c2[4] = c[4] * drjit::pow(s[0], 2) + c[5] * s[0] * s[1] + c[6] * s[0] * s[2] + c[7] * drjit::pow(s[1], 2) + c[8] * s[1] * s[2] + c[9] * drjit::pow(s[2], 2);
        ...
        c2[7] = c[4] * drjit::pow(t[0], 2) + c[5] * t[0] * t[1] + c[6] * t[0] * t[2] + c[7] * drjit::pow(t[1], 2) + c[8] * t[1] * t[2] + c[9] * drjit::pow(t[2], 2);
        c2[8] = 2 * c[4] * n[0] * t[0] + c[5] * (n[0] * t[1] + n[1] * t[0]) + c[6] * (n[0] * t[2] + n[2] * t[0]) + 2 * c[7] * n[1] * t[1] + c[8] * (n[1] * t[2] + n[2] * t[1]) + 2 * c[9] * n[2] * t[2];
        c2[9] = c[4] * drjit::pow(n[0], 2) + c[5] * n[0] * n[1] + c[6] * n[0] * n[2] + c[7] * drjit::pow(n[1], 2) + c[8] * n[1] * n[2] + c[9] * drjit::pow(n[2], 2);
        if (order > 2) {
            c2[10] = c[10] * drjit::pow(s[0], 3) + c[11] * drjit::pow(s[0], 2) * s[1] + c[12] * drjit::pow(s[0], 2) * s[2] + c[13] * s[0] * drjit::pow(s[1], 2) + c[14] * s[0] * s[1] * s[2] + c[15] * s[0] * drjit::pow(s[2], 2) + c[16] * drjit::pow(s[1], 3) +
                     c[17] * drjit::pow(s[1], 2) * s[2] + c[18] * s[1] * drjit::pow(s[2], 2) + c[19] * drjit::pow(s[2], 3);
            c2[11] = 3 * c[10] * drjit::pow(s[0], 2) * t[0] + c[11] * (drjit::pow(s[0], 2) * t[1] + 2 * s[0] * s[1] * t[0]) + c[12] * (drjit::pow(s[0], 2) * t[2] + 2 * s[0] * s[2] * t[0]) + c[13] * (2 * s[0] * s[1] * t[1] + drjit::pow(s[1], 2) * t[0]) +
                     c[14] * (s[0] * s[1] * t[2] + s[0] * s[2] * t[1] + s[1] * s[2] * t[0]) + c[15] * (2 * s[0] * s[2] * t[2] + drjit::pow(s[2], 2) * t[0]) + 3 * c[16] * drjit::pow(s[1], 2) * t[1] +
                     c[17] * (drjit::pow(s[1], 2) * t[2] + 2 * s[1] * s[2] * t[1]) + c[18] * (2 * s[1] * s[2] * t[2] + drjit::pow(s[2], 2) * t[1]) + 3 * c[19] * drjit::pow(s[2], 2) * t[2];
            c2[12] = 3 * c[10] * n[0] * drjit::pow(s[0], 2) + c[11] * (2 * n[0] * s[0] * s[1] + n[1] * drjit::pow(s[0], 2)) + c[12] * (2 * n[0] * s[0] * s[2] + n[2] * drjit::pow(s[0], 2)) + c[13] * (n[0] * drjit::pow(s[1], 2) + 2 * n[1] * s[0] * s[1]) +
                     c[14] * (n[0] * s[1] * s[2] + n[1] * s[0] * s[2] + n[2] * s[0] * s[1]) + c[15] * (n[0] * drjit::pow(s[2], 2) + 2 * n[2] * s[0] * s[2]) + 3 * c[16] * n[1] * drjit::pow(s[1], 2) +
                     c[17] * (2 * n[1] * s[1] * s[2] + n[2] * drjit::pow(s[1], 2)) + c[18] * (n[1] * drjit::pow(s[2], 2) + 2 * n[2] * s[1] * s[2]) + 3 * c[19] * n[2] * drjit::pow(s[2], 2);
            c2[13] = 3 * c[10] * s[0] * drjit::pow(t[0], 2) + c[11] * (2 * s[0] * t[0] * t[1] + s[1] * drjit::pow(t[0], 2)) + c[12] * (2 * s[0] * t[0] * t[2] + s[2] * drjit::pow(t[0], 2)) + c[13] * (s[0] * drjit::pow(t[1], 2) + 2 * s[1] * t[0] * t[1]) +
                     c[14] * (s[0] * t[1] * t[2] + s[1] * t[0] * t[2] + s[2] * t[0] * t[1]) + c[15] * (s[0] * drjit::pow(t[2], 2) + 2 * s[2] * t[0] * t[2]) + 3 * c[16] * s[1] * drjit::pow(t[1], 2) +
                     c[17] * (2 * s[1] * t[1] * t[2] + s[2] * drjit::pow(t[1], 2)) + c[18] * (s[1] * drjit::pow(t[2], 2) + 2 * s[2] * t[1] * t[2]) + 3 * c[19] * s[2] * drjit::pow(t[2], 2);
            c2[14] = 6 * c[10] * n[0] * s[0] * t[0] + c[11] * (2 * n[0] * s[0] * t[1] + 2 * n[0] * s[1] * t[0] + 2 * n[1] * s[0] * t[0]) + c[12] * (2 * n[0] * s[0] * t[2] + 2 * n[0] * s[2] * t[0] + 2 * n[2] * s[0] * t[0]) +
                     c[13] * (2 * n[0] * s[1] * t[1] + 2 * n[1] * s[0] * t[1] + 2 * n[1] * s[1] * t[0]) + c[14] * (n[0] * s[1] * t[2] + n[0] * s[2] * t[1] + n[1] * s[0] * t[2] + n[1] * s[2] * t[0] + n[2] * s[0] * t[1] + n[2] * s[1] * t[0]) +
                     c[15] * (2 * n[0] * s[2] * t[2] + 2 * n[2] * s[0] * t[2] + 2 * n[2] * s[2] * t[0]) + 6 * c[16] * n[1] * s[1] * t[1] + c[17] * (2 * n[1] * s[1] * t[2] + 2 * n[1] * s[2] * t[1] + 2 * n[2] * s[1] * t[1]) +
                     c[18] * (2 * n[1] * s[2] * t[2] + 2 * n[2] * s[1] * t[2] + 2 * n[2] * s[2] * t[1]) + 6 * c[19] * n[2] * s[2] * t[2];
            c2[15] = 3 * c[10] * drjit::pow(n[0], 2) * s[0] + c[11] * (drjit::pow(n[0], 2) * s[1] + 2 * n[0] * n[1] * s[0]) + c[12] * (drjit::pow(n[0], 2) * s[2] + 2 * n[0] * n[2] * s[0]) + c[13] * (2 * n[0] * n[1] * s[1] + drjit::pow(n[1], 2) * s[0]) +
                     c[14] * (n[0] * n[1] * s[2] + n[0] * n[2] * s[1] + n[1] * n[2] * s[0]) + c[15] * (2 * n[0] * n[2] * s[2] + drjit::pow(n[2], 2) * s[0]) + 3 * c[16] * drjit::pow(n[1], 2) * s[1] +
                     c[17] * (drjit::pow(n[1], 2) * s[2] + 2 * n[1] * n[2] * s[1]) + c[18] * (2 * n[1] * n[2] * s[2] + drjit::pow(n[2], 2) * s[1]) + 3 * c[19] * drjit::pow(n[2], 2) * s[2];
            c2[16] = c[10] * drjit::pow(t[0], 3) + c[11] * drjit::pow(t[0], 2) * t[1] + c[12] * drjit::pow(t[0], 2) * t[2] + c[13] * t[0] * drjit::pow(t[1], 2) + c[14] * t[0] * t[1] * t[2] + c[15] * t[0] * drjit::pow(t[2], 2) + c[16] * drjit::pow(t[1], 3) +
                     c[17] * drjit::pow(t[1], 2) * t[2] + c[18] * t[1] * drjit::pow(t[2], 2) + c[19] * drjit::pow(t[2], 3);
            c2[17] = 3 * c[10] * n[0] * drjit::pow(t[0], 2) + c[11] * (2 * n[0] * t[0] * t[1] + n[1] * drjit::pow(t[0], 2)) + c[12] * (2 * n[0] * t[0] * t[2] + n[2] * drjit::pow(t[0], 2)) + c[13] * (n[0] * drjit::pow(t[1], 2) + 2 * n[1] * t[0] * t[1]) +
                     c[14] * (n[0] * t[1] * t[2] + n[1] * t[0] * t[2] + n[2] * t[0] * t[1]) + c[15] * (n[0] * drjit::pow(t[2], 2) + 2 * n[2] * t[0] * t[2]) + 3 * c[16] * n[1] * drjit::pow(t[1], 2) +
                     c[17] * (2 * n[1] * t[1] * t[2] + n[2] * drjit::pow(t[1], 2)) + c[18] * (n[1] * drjit::pow(t[2], 2) + 2 * n[2] * t[1] * t[2]) + 3 * c[19] * n[2] * drjit::pow(t[2], 2);
            c2[18] = 3 * c[10] * drjit::pow(n[0], 2) * t[0] + c[11] * (drjit::pow(n[0], 2) * t[1] + 2 * n[0] * n[1] * t[0]) + c[12] * (drjit::pow(n[0], 2) * t[2] + 2 * n[0] * n[2] * t[0]) + c[13] * (2 * n[0] * n[1] * t[1] + drjit::pow(n[1], 2) * t[0]) +
                     c[14] * (n[0] * n[1] * t[2] + n[0] * n[2] * t[1] + n[1] * n[2] * t[0]) + c[15] * (2 * n[0] * n[2] * t[2] + drjit::pow(n[2], 2) * t[0]) + 3 * c[16] * drjit::pow(n[1], 2) * t[1] +
                     c[17] * (drjit::pow(n[1], 2) * t[2] + 2 * n[1] * n[2] * t[1]) + c[18] * (2 * n[1] * n[2] * t[2] + drjit::pow(n[2], 2) * t[1]) + 3 * c[19] * drjit::pow(n[2], 2) * t[2];
            c2[19] = c[10] * drjit::pow(n[0], 3) + c[11] * drjit::pow(n[0], 2) * n[1] + c[12] * drjit::pow(n[0], 2) * n[2] + c[13] * n[0] * drjit::pow(n[1], 2) + c[14] * n[0] * n[1] * n[2] + c[15] * n[0] * drjit::pow(n[2], 2) + c[16] * drjit::pow(n[1], 3) +
                     c[17] * drjit::pow(n[1], 2) * n[2] + c[18] * n[1] * drjit::pow(n[2], 2) + c[19] * drjit::pow(n[2], 3);
        }
    ...
    static Vector3f adjustRayDirForPolynomialTracing(Vector3f &inDir, const SurfaceInteraction3f &its, int polyOrder, Float polyScaleFactor, int channel = 0) {
        ...
        Float polyValue;
        Vector3f polyNormal;
        std::tie(polyValue, polyNormal) = evalPolyGrad(its.p, its.p, polyOrder, pX, pY, pZ, polyScaleFactor, false, inDir, its.polyCoeffs[channel]);
        polyNormal                      = drjit::normalize(polyNormal);
        Vector3f rotationAxis           = drjit::cross(its.sh_frame.n, polyNormal);
        if (dr::any_or<true>(dr::norm(rotationAxis) < 1e-8f)) { // If the normal and the polynomial normal are parallel
            return polyNormal;
        }
    ...
    */
    ...
    template <int order, typename T> static dr::Array<Float, nPolyCoeffs(order)> rotatePolynomialEigen(const T &c, Vector3f &s, Vector3f &t, Vector3f &n) {
        dr::Array<Float, nPolyCoeffs(order)> c2;
        // Eigen::Vector3f s(drjit::slice(sdr[0], 0), drjit::slice(sdr[1], 0), drjit::slice(sdr[2], 0));
        // Eigen::Vector3f t(drjit::slice(tdr[0], 0), drjit::slice(tdr[1], 0), drjit::slice(tdr[2], 0));
        // Eigen::Vector3f n(drjit::slice(ndr[0], 0), drjit::slice(ndr[1], 0), drjit::slice(ndr[2], 0));
