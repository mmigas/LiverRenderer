...
NAMESPACE_BEGIN(mitsuba)
    ...
    public:
        ...
        */
        Spectrum sampleAttenuatedEmitterDirect(DirectSamplingRecord<Float, Spectrum>& dRec,
                                               const SurfaceInteraction3f& its, int& interactions,
                                               const Point<Float, 2>& sample, Sampler* sampler = NULL) const;
         ...
         */
        Spectrum evalTransmittance(const Point3f& p1, bool p1OnSurface,
        ...
        std::vector<ref<Sensor>>& sensors() {
            return m_sensors;
        }

        /// Return the list of sensors (const version)
        const std::vector<ref<Sensor>>& sensors() const {
            return m_sensors;
        }
        ...
        const std::vector<ref<Emitter>>& emitters() const {
            return m_emitters;
        }

        /// Return the environment emitter (if any)
        const Emitter* environment() const {
        ...
        /// Return the list of shapes
        const std::vector<ref<Shape>>& shapes() const {
            return m_shapes;
        }
        ...
        const DynamicBuffer<ShapePtr>& shapes_dr() const {
            return m_shapes_dr;
        }

        /// Return the list of sensors as a Dr.Jit array
        const DynamicBuffer<SensorPtr>& sensors_dr() const {
            return m_sensors_dr;
        }
         ...
         */
        bool shapes_grad_enabled() const {
            return m_shapes_grad_enabled;
        };
    ...
    protected:
        ...
        /// Trace a ray
        MI_INLINE SurfaceInteraction3f ray_intersect_cpu(const Ray3f& ray, uint32_t ray_flags, Mask coherent,
                                                         Mask active) const;
