 ...
 */
template <typename Float, typename Spectrum> class MI_EXPORT_LIB VaeHelperEigen : public VaeHelper<Float, Spectrum> {
public:
    ...
    VaeHelperEigen(Float kernelEpsScale) { m_kernelEpsScale = kernelEpsScale; }
    ...
    inline bool prepare(const Scene *scene, const std::vector<ref<Shape>> &shapes, const Spectrum &sigmaT, const Spectrum &albedo, float g, float eta, const std::string &modelName, const std::string &absModelName, const std::string &angularModelName, const std::string &outputDir, int batchSize, const PolyFitConfig &pfConfig) override {
        ...
        if (m_config.configName == "FinalSharedLs/AbsSharedSimComplex") {
            ...
            Log(Info, "Prediction space is {0}", m_config.predictionSpace);
    ...
    virtual ScatterSamplingRecord<Float, Spectrum> sample(const Scene *scene, const Point3f &p, const Vector3f &d, const Vector3f &polyNormal, const Spectrum &sigmaT, const Spectrum &albedo, float g, float eta, Sampler *sampler, const SurfaceInteraction3f *its, bool projectSamples, int channel = 0) const override {
        // {
        ...
        if (m_config.predictionSpace == "AS") {
            ...
            // PolyUtils<Float, Spectrum>::projectPointsToSurface(scene, p, -d, sRec, shapeCoeffEigen, m_config
            //  .polyOrder,m_config.predictionSpace == "LS",fitScaleFactor, kernelEps);
        }
...
MI_EXTERN_CLASS(VaeHelperEigen)
