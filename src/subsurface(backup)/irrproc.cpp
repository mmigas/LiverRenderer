...
#include <mitsuba/render/range.h>#1#
#include "irrproc.h"

MTS_NAMESPACE_BEGIN

/* Parallel irradiance sampling implementation (worker) #1#
...
public:
    ...
    void process(const WorkUnit *workUnit, WorkResult *workResult,
        ...
        for (size_t i=0; i<positions.size(); ++i) {
            /* Create a fake intersection record #1#
...
ParallelProcess::EStatus IrradianceSamplingProcess::generateWork(WorkUnit *unit, int worker) {
    ...
    /* Reserve a sequence of at most 'granularity' samples #1#
