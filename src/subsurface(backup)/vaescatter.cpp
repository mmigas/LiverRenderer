...
public:
    ...
    Float FresnelMoment1(Float eta) const {
        ...
        Float return_float = drjit::if_stmt(std::make_tuple(eta), eta < 1, [&](const auto &) { return 0.45966f - 1.73965f * eta + 3.37668f * eta2 - 3.904945f * eta3 + 2.49277f * eta4 - 0.68441f * eta5; }, [&](const auto &) { return -4.61686f + 11.1136f * eta - 10.4646f * eta2 + 5.11455f * eta3 - 1.27198f * eta4 + 0.12746f * eta5; });
        return return_float;
    }
    ...
    void sampleOutgoingPosition(const Scene *scene, const SurfaceInteraction3f &its, const Vector3f &d, Sampler *sampler, ScatterSamplingRecord<Float, Spectrum> *sRec, Int32 nSamples) const {
        ...
        if (dr::any_or<true>(nSamples == 3)) {
            ...
            if (m_use_rgb) {
                ...
                int channel              = randomSampleChannel ? /*static_cast<Int32>(3 * sampler->next_1d())#1# 0 : its.sampledColorChannel;
                ...
                if (m_use_ptracer) {
                    ...
                    Vector3f polyNormal = PolyUtils<Float, Spectrum>::adjustRayDirForPolynomialTracing(
                        inDir, its, 3, PolyUtils<Float, Spectrum>::getFitScaleFactor(m_medium, channel), channel);
                ...
                Vector3f polyNormal =
                    PolyUtils<Float, Spectrum>::adjustRayDirForPolynomialTracing(inDir, its, 3, PolyUtils<Float, Spectrum>::getFitScaleFactor(m_medium));
    ...
    void preprocess(const ref<Scene> scene) override {
        ...
        (m_vaehelper.get())->prepare(scene.get(), scene->shapes(), m_sigmaT, m_albedo, m_g, m_eta, m_modelName, m_absModelName, m_angularModelName, m_outputDir, m_sssSamples, pfConfig);
        if (!m_use_ptracer) {
...
private:
    ...
    ref<VaeHelperEigen<Float, Spectrum>> m_vaehelper;
