...
template <typename Spectrum> void logError(int line, const Spectrum &value) {
...
// static StatsCounter avgPathLength("PTracer", "Average path length",
// EAverage);

template <typename Float, typename Spectrum> class VaeScatter : public Subsurface<Float, Spectrum> {
...
public:
    ...
    VaeScatter(const Properties &props) : Base(props) {
        ...
        Spectrum sigmaS = Spectrum(0.74f, 0.88f, 1.01f);
        Spectrum sigmaA = Spectrum(0.032f, 0.17f, 0.48f);
        Spectrum g      = Spectrum(0.0f);
        ...
        // m_albedo = props.get<Volume>("albedo", 0.5f);
        m_albedoTexture = props.texture<Texture>("albedo", 0.5f);
        m_sigmaTTexture = props.texture<Texture>("sigmaT", 0.5f);
        SurfaceInteraction3f si;
        m_albedo = m_albedoTexture->eval(si);
        m_sigmaT = m_sigmaTTexture->eval(si);
        m_g      = dr::slice(dr::mean(g), 0);
        ...
        m_modelName        = props.get<std::string>("modelname", "0487_FinalSharedLs7Mixed3_AbsSharedSimComplexMixed3");
        m_absModelName     = props.get<std::string>("absmodelname", "None");
        m_angularModelName = props.get<std::string>("angularmodelname", "None");
        m_outputDir        = props.get<std::string>("outputdir", "C:\\dev\\LiverRenderer\\pysrc\\outputs\\vae3d\\");
        ...
        if (m_use_ptracer) {
            ...
            // m_vaehelper = new VaeHelperPtracer(m_use_polynomials,
            // m_use_difftrans, m_disable_projection, m_kernelEpsScale);
            Log(Error, "VaeHelperPtracer not implemented");
        } else {
            m_vaehelper = new VaeHelper<Float, Spectrum>();
        }
    }

    virtual ~VaeScatter() override {
        Log(Info, "Done rendering VaeScatter, printing stats. Only accurate "
    ...
    inline Vector3f refract(const Vector3f &wi, Float cosThetaT, Float eta) const {
        Float scale = drjit::if_stmt(std::make_tuple(cosThetaT), cosThetaT < 0, [&](Float eta) { return -(1.0f / eta); }, [&](Float eta) { return -eta; });
    ...
    Float FresnelMoment1(Float eta) const {
        ...
        Float return_float = drjit::if_stmt(
            std::make_tuple(eta), eta < 1, [&](const auto &) { return 0.45966f - 1.73965f * eta + 3.37668f * eta2 - 3.904945f * eta3 + 2.49277f * eta4 - 0.68441f * eta5; },
            [&](const auto &) { return -4.61686f + 11.1136f * eta - 10.4646f * eta2 + 5.11455f * eta3 - 1.27198f * eta4 + 0.12746f * eta5; });
        return return_float;
    }
    ...
    Float Sw(const Vector3f &w) const { // TODO: When does this make a difference
        ...
        // return (1 - fresnelDielectricExt(Frame::cosTheta(w), cosThetaT,
        // m_eta)) / (c * M_PI);
        Float eta                                     = m_eta;
        auto [fresnelCoef, cosThetaT, eta_it, eta_ti] = fresnel(Frame<Float>::cos_theta(w), eta);
        return fresnelCoef;
    }

    std::pair<Ray3f, SurfaceInteraction3f> escapeObject(const Ray3f &ray, const Scene *scene) const {
        Ray3f escapeRay(ray.o, ray.d, ray.time);
        SurfaceInteraction3f rayIts;
        // Continue the ray until we eventually either hit an object or dont hit
        // anything
        Point3f absPoint = dr::abs(ray.o);
        Float eps        = math::ShadowEpsilon<Float> * (1.0f + dr::max(absPoint));
        for (int i = 0; i < m_maxSelfIntersections; ++i) {
            rayIts = scene->ray_intersect(escapeRay);
            if (dr::any_or<true>(rayIts.is_valid() && rayIts.shape->has_subsurface() && Frame<Float>::cos_theta(rayIts.wi) < 0)) {
                escapeRay = Ray3f(rayIts.p, ray.d, 100000.0f, ray.time);
            } else {
                break;
            }
        }
    ...
    bool isShadowedIgnoringSssObject(Ray3f ray, const Scene *scene, Float emitterDist) const {
        /*Float eps = math::ShadowEpsilon<Float> * (1.0f + dr::maximum(dr::abs(ray.o.x), dr::maximum(dr::abs(ray.o.y),
        dr::abs(ray.o.z))));
    ...
    static inline Vector3f reflect(const Vector3f &wi) { return Vector(-wi.x(), -wi.y(), wi.z()); }

    Spectrum LoImpl(const Scene *scene, Sampler *sampler, const SurfaceInteraction3f &its, const Vector3f &d, UInt32 depth, bool recursiveCall) const {
        its.predAbsorption = Spectrum(0.0f);

        const auto *integrator = dynamic_cast<const SamplingIntegrator<Float, Spectrum> *>(scene->integrator());
        if (sRecSingleTls.empty()) { // Not sure about the if statement
            // sRecSingleTls(1);
        }
        if (sRecBatchedTls.empty()) {
            // sRecBatchedTls = ScatterSamplingRecord<Float, Spectrum>(m_sssSamples);
        }
        if (sRecRgbTls.empty()) {
            // sRecRgbTls= ScatterSamplingRecord<Float, Spectrum>(3);
        }
        ...
        BSDFContext ctx;
        Mask active             = true;
        auto [bRec, bsdfWeight] = m_bsdf->sample(ctx, its, sampler->next_1d(active), sampler->next_2d(active), active);
        ...
        || (recursiveCall && ((bRec.sampledType & BSDF::ETransmission) != 0))) {
            ...
            return bsdfWeight * integrator->Li(RayDifferential(its.p,
        its.toWorld(bRec.wo), its.time), query);
        }*/

        Vector3f refractedD = -its.to_world(bRec.wo);
        // Trace a ray to determine depth through object, then decide whether we should use 0-scattering or multiple scattering Ray3f
        ...
        if (drjit::any_or<true>(sampler->next_1d() > 1 - drjit::exp(average * zeroScatterIts.t))) {
            ...
            // RadianceQueryRecord query(scene, sampler);
            //// query.newQuery(RadianceQueryRecord::ERadiance |
            // RadianceQueryRecord::EIntersection, its.shape->getExteriorMedium());
            // query.newQuery(RadianceQueryRecord::ERadiance, its.shape->getExteriorMedium());
            // query.depth = depth + 1;
            if (drjit::any_or<true>(depth > 10))
                return Spectrum(0.0f);

            if (drjit::any_or<true>(zeroScatterIts.shape->has_subsurface())) {
        ...
        Int32 nSamples                   = 1;
        dr::masked(nSamples, depth == 1) = 3;

        // ScatterSamplingRecord<Float, Spectrum> *sRec = sRecSingle;
        // dr::masked(sRec, nSamples == 3)              = sRecRgb;
        ScatterSamplingRecord<Float, Spectrum> *sRec = dr::any_or<true>(nSamples == 3) ? sRecRgb : sRecSingle;

        sampleOutgoingPosition(scene, its, refractedD, sampler, sRec, nSamples);
        Spectrum result(0.0f);
        Spectrum resultNoAbsorption(0.0f);
        int nMissed = 0;
        for (int i = 0; dr::any_or<true>(i < nSamples); ++i) {
            ...
            { // Sample 'bsdf' for outgoing ray
                ...
                if (m_disable_projection) {
                    // Escape the ray: Find new ray until the ray doesntintersect the SSS object form inside anymore
                ...
                // RadianceQueryRecord query(scene, sampler);
                // query.newQuery(RadianceQueryRecord::ERadiance | RadianceQueryRecord::EIntersection, its .shape->getExteriorMedium()); // exiting the current
                //
                //  shape
                // query.depth = depth + 1;
                ...
                if (drjit::any_or<true>(bsdfRayIts.is_valid())) { // If emitter was hit, we can apply MIS
                    if (dr::any_or<true>(bsdfRayIts.shape->is_emitter())) {
                        DirectionSample3f dRec;
                        // dRec.ref = outPosition;
                        // dRec.refN = normal;
                        // emitted = bsdfRayIts.shape->emitter->eval(-bsdfSampleRay.d);
                    ...
                    if (dr::any_or<true>(env)) {
                        DirectionSample3f dRec;
                        // dRec.refN = Vector3f(0.0f);
                        // emitted   = env->eval_environment(bsdfSampleRay);
                        // if (dr::any_or<true>(env->fillDirectSamplingRecord(dRec, bsdfSampleRay))) {
            ...
            { // Peform next event estimation and MIS with the path traced result
                auto [emitterRec, emitterSampleValue] = scene->sample_emitter_direction(its, sampler->next_2d(),
                ...
                if (!dr::any_or<true>(emitterSampleValue[0] == 0 || emitterSampleValue[1] == 0 || emitterSampleValue[2] == 0)) {
                    ...
                    if (drjit::any_or<true>(bsdfVal > 0)) {
                        ...
                        if (m_use_mis) {
                            Spectrum emitted = emitterSampleValue * bsdfVal * miWeight(emitterRec.pdf, bsdfPdf) * Sw(local.to_local(emitterRec.d));
        ...
        if (m_use_ptracer) {
            ...
            Float missedProjection = (Float) nMissed / (Float) nSamples;
            its.missedProjection   = (float) dr::sum_nested(missedProjection);
...
/*
 *
    Spectrum LoImplIter(const Scene *scene, Sampler *sampler, const SurfaceInteraction3f &initialIts, const Vector3f &initialD, UInt32 initialDepth = 0) const {
        // Accumulate the overall radiance here.
        Spectrum finalResult(0.0f);
        // Track throughput along the current ray path.
        Spectrum throughput(1.0f);

        // Current state variables.
        SurfaceInteraction3f its = initialIts;
        Vector3f d               = initialD;
        UInt32 depth             = initialDepth;

        // Loop until we hit a terminal condition.
        while (true) {
            // Reset predicted absorption.
            its.predAbsorption = Spectrum(0.0f);

            const auto *integrator = dynamic_cast<const SamplingIntegrator<Float, Spectrum> *>(scene->integrator());

            // (TLS records initialization omitted – assume they’re handled externally or not needed in the iterative version.)

            // Sample BSDF at the current interaction.
            BSDFContext ctx;
            Mask active             = true;
            auto [bRec, bsdfWeight] = m_bsdf->sample(ctx, its, sampler->next_1d(active), sampler->next_2d(active), active);
            Vector3f refractedD     = -its.to_world(bRec.wo);

            // Determine if the ray would exit without scattering.
            Ray3f zeroScatterRay(its.p, -refractedD, its.time);
            SurfaceInteraction3f zeroScatterIts = scene->ray_intersect(zeroScatterRay);
            if (drjit::any_or<true>(!zeroScatterIts.is_valid())) {
                // No valid intersection – terminate the loop.
                break;
            }

            // Compute average extinction rate if using RGB spectra.
            Float average = 0;
            if constexpr (is_rgb_v<Spectrum>) {
                average = (-m_sigmaT.r() + -m_sigmaT.g() + -m_sigmaT.b()) / 3;
            }
            // Decide probabilistically whether the ray passes through the object without scattering.
            if (drjit::any_or<true>(sampler->next_1d() > 1 - drjit::exp(average * zeroScatterIts.t))) {
                // Terminate if maximum depth is reached.
                if (drjit::any_or<true>(depth > 10))
                    break;
                // If the object has subsurface properties then “continue” the path
                // by updating the current interaction and direction.
                if (drjit::any_or<true>(zeroScatterIts.shape->has_subsurface())) {
                    throughput *= bsdfWeight;
                    // Set the new interaction and direction.
                    its = zeroScatterIts;
                    d   = refractedD;
                    ++depth;
                    // Loop to continue the path.
                    continue;
                } else {
                    // Otherwise, terminate the path.
                    break;
                }
            }

            // --- Scattering branch (non-zero-scatter) ---
            // Select sampling record type based on current depth.
            ScatterSamplingRecord<Float, Spectrum> sRecRgb[3];
            ScatterSamplingRecord<Float, Spectrum> sRecSingle[1];
            Int32 nSamples                               = 1;
            dr::masked(nSamples, depth == 1)             = 3;
            ...
            for (int i = 0; dr::any_or<true>(i < nSamples); ++i) {
                ...
                // Compute local throughput and adjust for absorption.
                Spectrum localThroughput = bsdfWeight * m_eta * m_eta;
                if (!m_disable_absorption)
                    localThroughput *= sRec[i].throughput;

                // --- Block 1: Evaluate a scattered ray using BSDF sampling and MIS ---
                {
                    const Vector3f &normal     = sRec[i].n;
                    const Point3f &outPosition = sRec[i].p;
                    // Create a local frame at the sampled point.
                    Frame3f frame(normal);
                    Vector3f bsdfLocalDir = warp::square_to_cosine_hemisphere(sampler->next_2d());
                    Float bsdfPdf         = warp::square_to_cosine_hemisphere_pdf(bsdfLocalDir);
                    Vector3f bsdfRayDir   = frame.to_world(bsdfLocalDir);
                    ...
                    if (m_disable_projection) {
                        // Escape the ray: find a ray that leaves the SSS object.
                        std::tie(bsdfSampleRay, bsdfRayIts) = escapeObject(bsdfSampleRay, scene);
                    }
                    bsdfRayIts                     = scene->ray_intersect(bsdfSampleRay);
                    bsdfRayIts.sampledColorChannel = sRec[i].sampledColorChannel;

                    // Evaluate illumination from an emitter or the environment.
                    Spectrum emitted(0.0f);
                    Float lumPdf = 0.0f;
                    if (drjit::any_or<true>(bsdfRayIts.is_valid())) {
                        if (dr::any_or<true>(bsdfRayIts.shape->is_emitter())) {
                            DirectionSample3f dRec;
                            //emitted = bsdfRayIts.shape->emitter->eval(-bsdfSampleRay.d);
                            //dRec.setQuery(bsdfSampleRay, bsdfRayIts);
                        ...
                        const Emitter<Float, Spectrum> *env = scene->environment();
                        if (drjit::any_or<true>(env)) {
                            DirectionSample3f dRec;
                            //emitted = env->eval_environment(bsdfSampleRay);
                            if (drjit::any_or<true>(env->fillDirectSamplingRecord(dRec, bsdfSampleRay))) {
                                lumPdf = scene->pdfEmitterDirect(dRec);
                            }
                        }
                    }
                    ...
                    if (m_use_mis) {
                        ...
                        Spectrum t = indirect * Sw(bsdfLocalDir);
                        // Alternative non-MIS accumulation could be performed here.
                        // result += t;
                        // resultNoAbsorption += t;
                    }
                }

                // --- Block 2: Next event estimation with MIS ---
                {
                    auto [emitterRec, emitterSampleValue] = scene->sample_emitter_direction(its, sampler->next_2d(), !m_disable_projection);

                    if (m_disable_projection) {
                        // Optionally test for shadowing here.
                    }

                    if (!dr::any_or<true>(emitterSampleValue[0] == 0 || emitterSampleValue[1] == 0 || emitterSampleValue[2] == 0)) {
                        // Use the emitter pointer.
                        EmitterPtr emitter  = static_cast<EmitterPtr>(emitterRec.emitter);
                        const Float bsdfVal = drjit::InvPi<Float> * drjit::maximum(drjit::dot(emitterRec.d, sRec[i].n), 0.0f);

                        Frame3f local(sRec[i].n);
                        if (drjit::any_or<true>(bsdfVal > 0)) {
                            Float bsdfPdf;
                            auto flag = emitter->flags() & (uint32_t) EmitterFlags::Surface;
                            bsdfPdf   = drjit::any_or<true>(flag) ? bsdfVal : 0;
                            if (m_use_mis) {
                                Spectrum emitted = emitterSampleValue * bsdfVal * miWeight(emitterRec.pdf, bsdfPdf) * Sw(local.to_local(emitterRec.d));
                                result += throughput * emitted;
                                resultNoAbsorption += emitted;
            ...
            // Accumulate the contribution from this scattering event.
            finalResult += throughput * (result / nSamples);

            // Terminate the loop after this branch; in a more complete iterative
            // implementation you might continue the loop if additional scattering
...
*/
    Spectrum sample(const Scene *scene, Sampler *sampler, const SurfaceInteraction3f &si, const Vector3f &d, UInt32 depth) const override {
        // Mask active = true;
        // dr::masked(dr::dot(si.sh_frame.n, d) < 0.0f, active);
        if (dr::any_or<true>(dr::dot(si.sh_frame.n, d) < 0.0f)) {
            return Spectrum(0.0f);
        }
        // return Spectrum(0.0f);
    ...
    void sampleOutgoingPosition(const Scene *scene, const SurfaceInteraction3f &its, const Vector3f &d, Sampler *sampler, ScatterSamplingRecord<Float, Spectrum> *sRec, Int32 nSamples) const {
        UnpolarizedSpectrum albedo = m_albedoTexture->eval(its);
        if (dr::any_or<true>(nSamples == 3)) {
            Vector3f inDir = -d;
            for (int i = 0; i < 3; ++i) {
                Vector3f inDir2       = inDir;
                Float polyScale       = PolyUtils<Float, Spectrum>::getFitScaleFactor(m_medium, i);
                Vector3f polyNormal   = PolyUtils<Float, Spectrum>::adjustRayDirForPolynomialTracing(inDir2, its, 3, polyScale, i);
                sRec[i]               = m_vaehelper->sample(scene, its.p, inDir2, polyNormal, m_sigmaT, m_albedo, m_g, m_eta, sampler, &its, true, i);
                Spectrum tmp          = sRec[i].throughput;
                sRec[i].throughput    = Spectrum(0.0f);
                sRec[i].throughput[i] = tmp[i] * 3.0f;
            }
        } else {
            assert(dr::any_or<true>(nSamples == 1));
            if (m_use_rgb) {
                // Sample random color channel to sample

                bool randomSampleChannel = its.sampledColorChannel < 0;
                int channel              = randomSampleChannel ? /*static_cast<Int32>(3 * sampler->next_1d())*/ 0 : its.sampledColorChannel;

                Vector3f inDir = -d;
                if (m_use_ptracer) {
                    sRec[0] = m_vaehelper->sample(scene, its.p, d, Vector3f(0.0f, 1.0f, 0.0f), m_sigmaT, albedo, m_g, m_eta, sampler, &its, true, channel);
                } else {
    ...
    void preprocess(const ref<Scene> scene) override {
        ...
        m_vaehelper->prepare(scene.get(), scene->shapes(), m_sigmaT, m_albedo, m_g, m_eta, m_modelName, m_absModelName, m_angularModelName, m_outputDir, m_sssSamples, pfConfig);
        if (!m_use_ptracer) {
            // if ML model is used, the config is well defined
    ...
    std::string to_string() const override {
        ...
        oss << "VaeScatter[" << std::endl << "  id = " << m_id << std::endl << "]";
        return oss.str();
    }
...
private:
    ...
    MediumParameters<Spectrum> m_medium;
    ref<VaeHelper<Float, Spectrum>> m_vaehelper;
    bool m_use_ptracer, m_use_difftrans, m_use_mis, m_use_polynomials, m_disable_projection, m_disable_absorption, m_visualize_invalid_samples, m_visualize_absorption, m_use_ptracer_direction, m_use_rgb;

    mutable double totalScatterTime      = 0.0;
    mutable double numScatterEvaluations = 0.0;

    std::vector<ScatterSamplingRecord<Float, Spectrum>> sRecSingleTls;
    std::vector<ScatterSamplingRecord<Float, Spectrum>> sRecBatchedTls;
    std::vector<ScatterSamplingRecord<Float, Spectrum>> sRecRgbTls;
};
