...
NAMESPACE_BEGIN(mitsuba)
    ...
    static void embree_intersect_packet(int* valid, void* geometryUserPtr,
        ...
        if (rtc_hit) {
            ...
            dr::store_aligned(
                rtc_hit->u, Float32P(dr::select(active, prim_uv.x(), dr::load_aligned<Float32P>(rtc_hit->u))));
            dr::store_aligned(
                rtc_hit->v, Float32P(dr::select(active, prim_uv.y(), dr::load_aligned<Float32P>(rtc_hit->v))));
            dr::store_aligned(rtc_hit->geomID,
                              dr::select(active, UInt32P(geomID), dr::load_aligned<UInt32P>(rtc_hit->geomID)));
            dr::store_aligned(rtc_hit->primID,
                              dr::select(active, UInt32P(primID), dr::load_aligned<UInt32P>(rtc_hit->primID)));
            dr::store_aligned(rtc_hit->instID[0],
                              dr::select(active, UInt32P(instID), dr::load_aligned<UInt32P>(rtc_hit->instID[0])));
        } else {
    ...
    MI_VARIANT
    RTCGeometry Shape<Float, Spectrum>::embree_geometry(RTCDevice device) {
        if constexpr (!dr::is_cuda_v<Float>) {
    ...
    MI_VARIANT
    void Shape<Float, Spectrum>::optix_prepare_geometry() {
    ...
    MI_VARIANT
    void Shape<Float, Spectrum>::optix_prepare_ias(const OptixDeviceContext& /*context*/,
    ...
    MI_VARIANT
    void Shape<Float, Spectrum>::optix_build_input(OptixBuildInput& build_input) const {
    ...
    MI_VARIANT
    Float Shape<Float, Spectrum>::pdf_direction(const Interaction3f& /*it*/,
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::SilhouetteSample3f
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::Point3f
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::PreliminaryIntersection3f
...
#define MI_DEFAULT_RAY_INTERSECT_PACKET(N)                                                          \
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::Mask
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::UnpolarizedSpectrum
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::Color3f
    ...
    MI_VARIANT
    typename Shape<Float, Spectrum>::ScalarSize
    ...
    MI_VARIANT
    void Shape<Float, Spectrum>::traverse(TraversalCallback* callback) {
        ...
        callback->put_parameter("silhouette_sampling_weight", m_silhouette_sampling_weight,
    ...
    MI_VARIANT
    bool Shape<Float, Spectrum>::parameters_grad_enabled() const {
        return false;
    }

    MI_VARIANT
    void Shape<Float, Spectrum>::initialize() {
        if constexpr (dr::is_jit_v<Float>) {
            bool is_bspline_curve = (shape_type() == +ShapeType::BSplineCurve);
            bool is_linear_curve = (shape_type() == +ShapeType::LinearCurve);
    ...
    MI_VARIANT
    std::string Shape<Float, Spectrum>::get_children_string() const {
