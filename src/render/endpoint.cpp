...
NAMESPACE_BEGIN(mitsuba)
    MI_VARIANT
    Endpoint<Float, Spectrum>::Endpoint(const Properties& props) : m_id(props.id()) {
    ...
    MI_VARIANT
    void Endpoint<Float, Spectrum>::set_scene(const Scene*) {
    }
    ...
    MI_VARIANT
    std::pair<typename Endpoint<Float, Spectrum>::Wavelength, Spectrum>
    ...
    std::pair<Spectrum, DirectSamplingRecord<Float, Spectrum>> Endpoint<Float, Spectrum>::sampleDirect(
        const DirectSamplingRecord<Float, Spectrum>& dRec,
        const SurfaceInteraction3f& its,
        const Point<Float, 2>& sample) const {
        NotImplementedError("sampleDirect");
    }

    MI_VARIANT
    Float Endpoint<Float, Spectrum>::pdf_position(
        const PositionSample3f& /*ps*/, Mask /*active*/) const {
        NotImplementedError("pdf_position");
    }

    MI_VARIANT
    Spectrum Endpoint<Float, Spectrum>::pdf_wavelengths(
    ...
    MI_VARIANT
    void Endpoint<Float, Spectrum>::traverse(TraversalCallback* callback) {
        if (m_medium)
            callback->put_object("medium", m_medium.get(), +ParamFlags::Differentiable);
    }

    MI_VARIANT
    void Endpoint<Float, Spectrum>::parameters_changed(const std::vector<std::string>& keys) {
