...
NAMESPACE_BEGIN(mitsuba)
        ...
        for (auto& [k, v]: props.objects()) {
            ...
            if (shape) {
                ...
                if (emitter->is_environment()) {
                    if (m_environment)
                        Throw(
                        "Only one environment emitter can be specified per scene.");
    ...
    void Scene<Float, Spectrum>::update_emitter_sampling_distribution() {
        ...
        if (non_uniform_sampling) {
            std::unique_ptr<ScalarFloat[]> sample_weights(
                new ScalarFloat[n_emitters]);
    ...
    MI_VARIANT
    void Scene<
        Float, Spectrum>::update_silhouette_sampling_distribution() {
        ...
        for (size_t i = 0; i < n_shapes; ++i) {
            ...
            if (grad_enabled && (weight > 0.f)) {
                ...
                bool has_interior = has_flag(
                    types, DiscontinuityFlags::InteriorType);
                bool has_perimeter = has_flag(
                    types, DiscontinuityFlags::PerimeterType);
    ...
    MI_VARIANT
    typename Scene<Float, Spectrum>::SurfaceInteraction3f
    ...
    MI_VARIANT
    typename Scene<Float, Spectrum>::PreliminaryIntersection3f
    Scene<Float, Spectrum>::ray_intersect_preliminary(
    ...
    MI_VARIANT
    typename Scene<Float, Spectrum>::Mask
    ...
    MI_VARIANT
    std::tuple<typename Scene<Float, Spectrum>::UInt32, Float, Float>
    Scene<Float, Spectrum>::sample_emitter(Float index_sample, Mask active) const {
        ...
        if (m_emitter_distr != nullptr) {
            auto [index, reused_sample, pmf] = m_emitter_distr->sample_reuse_pmf(
                index_sample);
    ...
    MI_VARIANT
    Float Scene<Float, Spectrum>::pdf_emitter(UInt32 index,
                                              Mask active) const {
        if (m_emitter_distr == nullptr)
    ...
    MI_VARIANT
    std::tuple<typename Scene<Float, Spectrum>::Ray3f, Spectrum,
    ...
    Scene<Float, Spectrum>::sample_emitter_direction(
        ...
        if (emitter_count > 1 || (emitter_count == 1 && !vcall_inline)) {
            ...
            auto [index, emitter_weight, sample_x_re] = sample_emitter(
                sample.x(), active);
            sample.x() = sample_x_re;

            // Sample a direction towards the emitter
            EmitterPtr emitter = dr::gather<EmitterPtr>(
                m_emitters_dr, index, active);
            ...
            std::tie(ds, spec) = m_emitters[0]->sample_direction(
                ref, sample, active);
    ...
    Scene<Float, Spectrum>::pdf_emitter_direction(const Interaction3f& ref,
        ...
        if (m_emitter_distr == nullptr)
            ...
            emitter_pmf = ds.emitter->sampling_weight() * m_emitter_distr->
                          normalization();
        return ds.emitter->pdf_direction(ref, ds, active) * emitter_pmf;
    }

    MI_VARIANT
    Spectrum Scene<Float, Spectrum>::eval_emitter_direction(
    ...
    MI_VARIANT
    typename Scene<Float, Spectrum>::SilhouetteSample3f
    Scene<Float, Spectrum>::sample_silhouette(const Point3f& sample_,
        ...
        if (has_interior != has_perimeter) {
            // Only one discontinuity type
    ...
    MI_VARIANT
    typename Scene<Float, Spectrum>::Point3f
    Scene<Float, Spectrum>::invert_silhouette_sample(const SilhouetteSample3f& ss,
        ...
        Mask shape_has_both_types =
                ss.shape->silhouette_discontinuity_types() == (uint32_t)
                DiscontinuityFlags::AllTypes;
        Mask is_interior =
    ...
    void Scene<
        Float, Spectrum>::wake_up() {
        for (auto& shape: m_shapes) {
            if (shape->has_subsurface()) {
                shape->subsurface()->wake_up();
            }
        }
    }
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::traverse(TraversalCallback* callback) {
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::parameters_changed(
        const std::vector<std::string>&/*keys*/) {
        if (m_environment)
            m_environment->set_scene(this);
        // TODO use parameters_changed({"scene"})
    ...
    MI_VARIANT
    std::string Scene<Float, Spectrum>::to_string() const {
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::static_accel_initialization() {
        if constexpr (dr::is_cuda_v<Float>)
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::static_accel_shutdown() {
        if constexpr (dr::is_cuda_v<Float>)
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::clear_shapes_dirty() {
        for (auto& s: m_shapes)
            s->m_dirty = false;
        for (auto& s: m_shapegroups)
            s->m_dirty = false;
    ...
    MI_VARIANT
    void Scene<Float, Spectrum>::static_accel_initialization_cpu() {
    }

    MI_VARIANT
    void Scene<Float, Spectrum>::static_accel_shutdown_cpu() {
    }
