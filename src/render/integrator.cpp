...
NAMESPACE_BEGIN(mitsuba)
    ...
    MI_VARIANT
    typename Integrator<Float, Spectrum>::TensorXf
    ...
    MI_VARIANT
    std::vector<std::string> Integrator<Float, Spectrum>::aov_names() const {
        return {};
    }

    MI_VARIANT
    void Integrator<Float, Spectrum>::cancel() {
        m_stop = true;
    }
    ...
    MI_VARIANT
    typename SamplingIntegrator<Float, Spectrum>::TensorXf
    ...
    MI_VARIANT
    Spectrum SamplingIntegrator<Float, Spectrum>::E(const Scene* scene, const SurfaceInteraction3f& its,
                                                    Sampler* sampler, int nSamples,
                                                    bool includeIndirect) const {
        Spectrum E(0.0f);
        RadianceQueryRecord<Float, Spectrum> query(scene, sampler);
        DirectSamplingRecord<Float, Spectrum> dRec(its);
        Frame3f frame(its.sh_frame.n);

        int maxIntermediateInteractions = -1;
        Spectrum directRadiance = scene->sampleAttenuatedEmitterDirect(
            dRec, its, maxIntermediateInteractions, query.nextSample2D());
        sampler->seed(0);
        for (int i = 0; i < nSamples; i++) {
            /* Sample the direct illumination component */

            if (!drjit::any_or<true>(directRadiance.x() == 0.f) && !drjit::any_or<true>(directRadiance.y() == 0.f) &&
                !drjit::any_or<true>(directRadiance.z() == 0.f)) {
                Float dp = drjit::dot(dRec.d, its.sh_frame.n);
                if (drjit::all_nested(dp > 0)) {
                    E += directRadiance * dp;
                }
            }
            ...
            if (includeIndirect) {
                ...
                Vector d = frame.toWorld(warp::square_to_cosine_hemisphere(query.nextSample2D()));
                ++query.depth;
                query.medium = medium;
    ...
    MI_VARIANT
    AdjointIntegrator<Float, Spectrum>::~AdjointIntegrator() {
    }

    MI_VARIANT
    typename AdjointIntegrator<Float, Spectrum>::TensorXf
