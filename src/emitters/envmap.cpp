...
NAMESPACE_BEGIN(mitsuba)
    ...
    public:
        ...
        std::pair<Spectrum, DirectSamplingRecord<Float, Spectrum>> sampleDirect(
            const DirectSamplingRecord<Float, Spectrum>& dRec,
            const SurfaceInteraction3f& its,
            const Point2f& sample) const override {
            const Transform3f& transform = m_to_world.value();

            /* Sample a direction from the environment map */
            Mask active = true;
            auto [direction, value] = sample_direction(its, sample, active);
            Vector3f d = direction.d;
            Float pdf = direction.pdf;
            ...
            Ray3f ray(its.p, d, its.time);
            Float nearT, farT;
            auto hit = m_bsphere.ray_intersect(ray);

            DirectSamplingRecord<Float, Spectrum> directSampling;
            directSampling.pdf = pdf;
            directSampling.p = direction.p;
            directSampling.n = normalize(m_bsphere.center - dRec.p);
            directSampling.dist = direction.dist;
            directSampling.d = direction.d;
            //dRec.measure = ESolidAngle;

            return {value / pdf, directSampling};
        }
